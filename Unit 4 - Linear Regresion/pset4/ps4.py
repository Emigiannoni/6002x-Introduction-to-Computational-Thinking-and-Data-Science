...
def generate_models(x, y, degs):
    ...
    for deg in degs:
        ...
        array = np.asarray(pylab.polyfit(x, y, deg))
...
def r_squared(y, estimated):
    ...
    N = len(y)
    mean = np.array(y).sum() / N
    numeradores = []
    denominadores = []
    
    for i in range(N):
        
       numerador = y[i] - estimated[i]
       numeradores.append(numerador**2)
       denominador = y[i] - mean
       denominadores.append(denominador**2)

    numeradores = np.array(numeradores)
    denominadores = np.array(denominadores)
    
    R2 = 1 - ((numeradores.sum())/(denominadores.sum()))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        N = len(model)
        x_model = list(range(min(years), max(years) + 1))
        y_model = []
        ...
        for x in x_model:         
            ...
            for n in range(N):
                if (N - n) != 0:
                    value += model[n] * (x ** ((N - 1) - n))
                else:
                    value += n
            y_model.append(value)        

        R2 = r_squared(temps, y_model)
        
        title = 'Grade ' + str(N-1) + ' aproximation, with R^2 = ', str(round(R2, 3))
        
        pylab.figure('model')
        pylab.clf()
        pylab.plot(years, temps, 'bo')
        pylab.plot(x_model, y_model, 'r-')
        pylab.title(title)
        pylab.xlabel('Year')
